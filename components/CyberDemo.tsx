...
interface CyberDemoProps {
  lang: 'en' | 'zh';
  onDemoComplete?: () => void;
}

const CyberDemo: React.FC<CyberDemoProps> = ({ lang, onDemoComplete }) => {
  const [stage, setStage] = useState<'boot' | 'home' | 'complete'>('boot');
  ...
  }, [stage, t.boot_logs.length]);

  const handleInitiate = () => {
    setStage('home');
  };

  // Boot Screen Component for Demo (exact copy of original but without fixed positioning)
  const BootScreenInFrame = () => {
    const [logIndex, setLogIndex] = useState(0);
    const [isReady, setIsReady] = useState(false);
    const [progress, setProgress] = useState(0);
    ...
    if (!isInitialized) {
      return (
        <div className="w-full h-full bg-black flex items-center justify-center">
          <div className="text-[#00f3ff] text-xs animate-pulse">Initializing...</div>
        </div>
      );
    }

    return (
      <div className="w-full h-full bg-black flex flex-col items-center justify-center p-6 font-tech overflow-hidden">
        {/* Language Switcher */}
        <div className="absolute top-8 right-8 z-50 flex gap-4">
          <button className="text-xs p-2 border border-white/10 text-white/30">
            [ EN ]
          </button>
          <button className="text-xs p-2 border border-white/10 text-white/30">
            [ 繁 ]
        ...
        <div className="w-full max-w-lg space-y-8 relative z-10">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-[10px] text-[#00f3ff]">
              <span className="uppercase tracking-widest">{lang === 'zh' ? '初始化進度' : 'Initialization Progress'}</span>
  ...
  return (
    <div className="w-full h-full bg-black text-white relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        {stage === 'boot' && <BootScreenInFrame />}
        {stage === 'home' && (
          <div className="h-auto pb-20">
            <HomePage 
              lang={lang} 
              onInitiate={() => setStage('complete')} 
            />
            {/* 添加額外空間確保滾動 */}
            <div className="h-96 flex items-center justify-center">
              <div className="text-center">
                <p className="text-gray-500 text-sm mb-4">Scroll down for more content</p>
        ...
        {stage === 'complete' && (
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-3xl font-bold text-[#00f3ff] mb-4">DEMO COMPLETE</h2>
              <p className="text-white/60 mb-6">Thank you for experiencing the Cyber-Forge 2077 demo!</p>
              <button 
                onClick={onDemoComplete}
                className="px-6 py-3 border border-[#00f3ff] text-[#00f3ff] rounded hover:bg-[#00f3ff] hover:text-black transition-colors"
              >
                Back to Showcase
              </button>
            </div>
